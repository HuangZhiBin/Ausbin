...
extension NSObject {
    ...
    ///
    /// - Returns: 属性名和类型的字典
    func asb_getProperties(_ theClass: AnyClass? = nil) -> Dictionary<String, String>? {
        var count = UInt32()
        guard let properties = class_copyPropertyList((theClass == nil ? self.classForCoder : theClass), &count) else { return nil }
        ...
        if theClass == nil {
            if(self.superclass != nil && NSStringFromClass((self.superclass?.class())!) != NSStringFromClass(NSObject.classForCoder())){
                let typesOfSuper = self.asb_getProperties(self.superclass);
            ...
            if(theClass?.superclass != nil && NSStringFromClass((theClass?.superclass())!) != NSStringFromClass(NSObject.classForCoder())){
                let typeOfSuper = self.asb_getProperties(theClass?.superclass());
